...
function ExperimentTotal({ experimentTypeName }: Props) {
  ...
  return (
    <>
      ...
      {total.state === 'hasData' && (
        <Link
          href={`/explore/interactive/data/${EXPERIMENT_DATA_TYPES[experimentTypeName].name}`}
          key={EXPERIMENT_DATA_TYPES[experimentTypeName].title}
        ...
        >
          <span className="font-light">{EXPERIMENT_DATA_TYPES[experimentTypeName].title}</span>
          <span>
