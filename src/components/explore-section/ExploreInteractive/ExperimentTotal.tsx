...
function ExperimentTotal({ experimentTypeName }: Props) {
  ...
  return (
    <>
      ...
      {total.state === 'hasData' && (
        <Link
          href={`/explore/interactive/data/${DATA_TYPES[experimentTypeName].name}`}
          key={DATA_TYPES[experimentTypeName].title}
        ...
        >
          <span className="font-light">{DATA_TYPES[experimentTypeName].title}</span>
          <span>
