...
export default async function retrievePapersList({ virtualLabId, projectId, accessToken }:
  { virtualLabId: string; projectId: string; accessToken: string }
): Promise<{
  total: number;
  papers: Array<PaperResource>;
}> {
  const query = getPaperListQuery(virtualLabId, projectId);
  const apiUrl = composeUrl('view', nexus.defaultESViewId, { viewType: 'es', org: virtualLabId, project: projectId });
