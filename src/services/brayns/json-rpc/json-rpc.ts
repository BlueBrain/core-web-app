...
export default class JsonRpcService {
  ...
  constructor(hostAndPort: string, private readonly secure: boolean) {
  ...
  readonly exec = async (
    ...
    try {
      ...
      if (this.isError(data)) {
        logError('JsonRpc query failure!', data);
        logError('   Address:', this.host, this.port);
        /**
         * We throw a JsonRpcQueryFailure here.
         * This way the client can tell the difference between
         * a local error and an exception coming from the remote
         * server.
         * They will use `isJsonRpcQueryFailure()` for this.
         */
      ...
      logError(`Error while calling entry point "${entryPointName}":`, param);
      logError(ex);
      throw ex;
    }
  };
  ...
  execLongTask(
    ...
    const promise = this.makeLongTaskPromise(id, entryPointName, param, stop, ws, message, chunk);
    return {
      promise,
      cancel: () => {
  ...
  private handleResponse(id: string, params: SerializableData) {
    ...
    traceEntryPoint(`<<< ${getDuration(query)}`, query.entryPointName, params);
    this.pendingQueries.delete(id);
